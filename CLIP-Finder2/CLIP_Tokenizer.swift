...
class CLIPTokenizer {
    ...
    init(bpePath: String, contextLength: Int = 77) {
...
//        var vocab = Array(self.byteEncoder.values)
        vocab += vocab.map { $0 + "</w>" }
        ...
        self.tokensToIds = Dictionary(uniqueKeysWithValues: zip(vocab, 0..<vocab.count))
        ...
        self.vocabSize = self.tokensToIds.count
    ...
    func byteEncode(text: String) -> [String] {
        ...
        // print("esp: \(String(describing: byteEncoder[32]))")
...
//        }
        ...
        if let cachedToken = cache[token] {
            print("Cache hit for token: \(token)")
            return cachedToken
        }
        ...
        while true {
            ...
            if word.count == 1 {
                ...
                print("Updated pairs: \(pairs)")
            }
        }
        
        let result = word.joined(separator: " ")
        cache[token] = result
        print("BPE result: \(result)")
        return result
    }
    ...
    func tokenize(texts: [String]) -> [[Int]] {

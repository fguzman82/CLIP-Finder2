...
CLIP-Profiler is an iOS application that leverages advanced AI models to perform image and text similarity searches. It utilizes two CoreML models optimized for the Apple Neural Engine, ensuring efficient on-device processing. The app allows users to search for images from the gallery of photos and provides a user-friendly interface for searching and profiling, with all searches being completely offline, making the most of Apple's cutting-edge AI capabilities.
